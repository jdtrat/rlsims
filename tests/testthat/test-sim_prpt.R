...
test_that("prpt simulation is consistent", {

  local_edition(3)

  set.seed(18)
  new <- simulate_prpt_iaps(task_type = "prpt")

  expect_equal(prpt_sim_seed18, new)

})

test_that("iaps simulation is consistent", {

  set.seed(18)
  new <- simulate_prpt_iaps(task_type = "iaps")

  expect_equal(iaps_sim_seed18, new)

})


test_that("prpt reward structure is correct", {

  reward_struc <- matrix(nrow = 6,
                         ncol = 2)

  reward_struc[1,] <- c(1,2.5)
  reward_struc[2,] <- c(1,1.5)
  reward_struc[3,] <- c(1,0.5)
  reward_struc[4,] <- c(-1,-1.25)
  reward_struc[5,] <- c(-1,-0.75)
  reward_struc[6,] <- c(-1,-0.25)



  set.seed(18)
  simulate_prpt_iaps(task_type = "prpt")
  prpt_reward_struc <- prpt_iaps$reward_structure

  expect_equal(reward_struc, prpt_reward_struc)

})


test_that("iaps reward struc is correct", {

  reward_struc <- matrix(nrow = 6,
                         ncol = 2)

  reward_struc[1,] <- c(iaps_rewards$`1.00`, iaps_rewards$`2.50`)
  reward_struc[2,] <- c(iaps_rewards$`1.00`, iaps_rewards$`1.50`)
  reward_struc[3,] <- c(iaps_rewards$`1.00`, iaps_rewards$`0.50`)
  reward_struc[4,] <- c(iaps_rewards$`-1.00`, iaps_rewards$`-1.25`)
  reward_struc[5,] <- c(iaps_rewards$`-1.00`, iaps_rewards$`-0.75`)
  reward_struc[6,] <- c(iaps_rewards$`-1.00`, iaps_rewards$`-0.25`)

  set.seed(18)
  simulate_prpt_iaps(task_type = "iaps")
  iaps_reward_struc <- prpt_iaps$reward_structure
