...
test_that("simulation is required", {
  expect_error(both_set$get_tidy_pe_data(),
               regexp = "Cannot access prediction error data")
})

test_that("simulation is required", {
  expect_s3_class(simulated$get_tidy_pe_data(),
                  class = "data.frame")

  expect_named(simulated$get_tidy_pe_data(),
...
test_that("appending trial zero works", {
  ...
  expect_equal(
    simulated$get_tidy_pe_data(add_trial_zero = TRUE, trial_zero_value = 0)[1:10,],
    data.frame(trial = 0, episode = 1:10, value = 0)
  )

  expect_equal(
    simulated$get_tidy_pe_data(add_trial_zero = FALSE, trial_zero_value = 0)[1:10,],
    data.frame(trial = 1, episode = 1:10, value = c(rep(0, 7), 1, 0, 0))
  )

  expect_equal(
    simulated$get_tidy_pe_data(add_trial_zero = TRUE, trial_zero_value = 1:10)[1:10,],
